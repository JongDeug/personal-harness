...
6. 생성 완료 메시지 + RC 중 버그 발견 시 `/rc-fix {issue-key}` 사용 안내
...
RC를 PR을 통해 main과 develop에 머지하고 태그를 생성한다. 모든 머지는 PR로 진행하여 GitHub에서 이력을 확인할 수 있도록 한다.
...
3. **main으로 PR 생성:**
   ...
   gh pr create --base main --head rc/{version} \
     ...
     --body "RC {version} → main 배포"
   ```
4. PR URL 출력 후 사용자에게 머지 요청. 사용자가 머지 완료했다고 알릴 때까지 대기
5. **머지 확인 후 태깅:**
   ```
