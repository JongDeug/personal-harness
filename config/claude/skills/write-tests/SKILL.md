...
3. **테스트 디렉토리 확인**: 프로젝트 루트에 `test/` 폴더가 없으면 생성한다
4. **기존 테스트 확인**: `test/` 폴더 안에 대응하는 테스트 파일이 있는지 확인
   - 있으면: 기존 테스트 구조를 따르고 누락된 케이스만 추가
   - 없으면: 아래 템플릿에 따라 새로 작성
5. **테스트 파일 위치**: 소스 파일의 경로 구조를 `test/` 아래에 그대로 미러링한다
   - 예) `src/modules/channels/core/id-adapter.util.ts` → `test/modules/channels/core/id-adapter.util.spec.ts`
   - 예) `src/common/utils/language-detector.util.ts` → `test/common/utils/language-detector.util.spec.ts`
6. **테스트 작성**: 정책(3종 케이스, Mock 규칙, Mock 타입 규칙)에 따라 작성
7. **테스트 실행**: 작성 후 실행하여 통과 확인
...
- **기능 코드 수정 시 테스트 동시 수정**: 반환 타입, DTO 구조, 파라미터 등을 변경하면 해당 서비스를 mock하는 모든 테스트 파일을 검색하여 mock 데이터와 assertion도 반드시 함께 수정한다
...
- 테스트 파일 위치: 프로젝트 루트 `test/` 폴더 아래에 소스 경로를 미러링하여 작성 (`*.spec.ts`)
- e2e 테스트는 `test/e2e/` 디렉토리에 `*.e2e-spec.ts`로 작성
- **mock 변수에 `any`나 `Record<string, any>` 타입 사용 금지**. `jest-mock-extended`의 `mock<T>()`와 `MockProxy<T>`를 사용하여 타입 안전한 mock을 생성한다:
  ```typescript
  ...
  // ✅ Good - 반환 타입 불일치 시 컴파일 에러
  let mockService: MockProxy<CustomerService>;
  let mockRepo: MockProxy<Repository<Tcustomer>>;
  ```

### Unit Test — 기본 구조

`Test.createTestingModule()`에 실제 클래스를 등록하고, 외부 의존성만 `mock<T>()`로 생성하여 `useValue`로 주입한다.
`jest-mock-extended`의 `mock<T>()`는 모든 메서드를 자동으로 `jest.fn()`으로 생성하므로 필요한 메서드만 `mockResolvedValue` 등으로 반환값을 설정하면 된다.
...
describe('CustomerService', () => {
  ...
  let mockRepo: MockProxy<Repository<Tcustomer>>;
  let mockCommonService: MockProxy<CommonService>;

  beforeEach(async () => {
    mockRepo = mock<Repository<Tcustomer>>();
    mockCommonService = mock<CommonService>();

    const moduleRef: TestingModule = await Test.createTestingModule({
      providers: [
        CustomerService,
        { provide: getRepositoryToken(Tcustomer, 'trustnhopedb'), useValue: mockRepo },
  ...
  describe('getCustomers', () => {
    it('[Happy Path] 고객 목록 반환', async () => {
      // mockRepo의 모든 메서드는 이미 jest.fn()으로 생성됨
      mockRepo.createQueryBuilder.mockReturnValue(mockQb as any);

      const result = await service.getCustomers({ page: 1, take: 10 });

      expect(result.list).toHaveLength(1);
    });

    it('[Edge Case] 검색 조건 없어도 정상 반환', async () => { ... });
