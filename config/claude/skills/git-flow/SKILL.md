...
description: >
  ...
  - /release-version {version} — 실배포 1단계 (배포 태그 생성 + 실서버 배포 검증 + main 머지)
  - /release-complete — 실배포 2단계 (GitHub Releases 릴리즈 노트 드래프트 게시)
  - /release-sync — 실배포 3단계 (develop 동기화 + main↔develop diff 확인)
...
| 배포 ① | `/release-version 2.1.0.0` | 배포 태그 생성 → 실서버 배포 검증 → main 머지 |
| 배포 ② | `/release-complete` | GitHub Releases 릴리즈 노트 드래프트 게시 |
| 배포 ③ | `/release-sync` | develop 동기화 + main↔develop diff 확인 |
| 긴급 수정 | `/hotfix 2.1.0.0` | main에서 hotfix 브랜치 생성 |
| 긴급 수정 완료 | `/finish-hotfix` | main 머지 + 태깅 + RC rebase |

**일반적인 흐름**: `/feat` → 구현 → `/finish-feat` → PR 머지 → `/start-rc` → QA → `/rc-notes` → `/release-version` → `/release-complete` → `/release-sync`
...
9. **출력 끝에 안내**:
   ...
   배포 플로우에서는 `/release-complete`가 이 노트를 자동으로 드래프트 게시합니다.
   ```
...
실배포 1단계. RC에서 배포 태그를 먼저 생성해 이미지 빌드/실서버 배포까지 검증한 후, main으로 PR 머지한다.

**핵심 원칙**: 배포 태그 생성 → 이미지 빌드 → 실서버(KT) 배포 성공 확인 → main 머지.
배포 검증 이전에는 **절대 main을 변경하지 않는다.** 실배포가 실패하면 해당 태그는 폐기하고 `/rc-fix`로 수정 후 새 태그를 따서 재시도한다.

배포 태그는 RC 버전의 **minor를 +1하고 hotfix·test를 0으로 리셋**한 버전을 사용한다.
(예: `rc/2.1.0.3` → 배포 태그 `2.2.0.0`)

#### Phase 1 — 배포 태그 생성 (RC 브랜치 작업만)
...
4. **RC 브랜치에서 package.json 버전 업데이트 및 태그 생성:**
   ...
   ```
   (예: `2.2.0.0`)
5. **이미지 빌드 및 KT 실서버 배포 안내 출력:**
   ```
   ✅ 배포 태그 {release_version} 생성 완료 (RC 브랜치 push 됨)

   다음 단계 (사용자 수동 진행):
   1. 이 태그로 이미지를 빌드해 주세요
   2. KT 실서버에 배포를 진행해 주세요
   3. 배포가 **성공적으로 검증**되면 "배포 성공"이라고 알려주세요

   ⚠️ 배포 성공 확인 전까지 main은 건드리지 않습니다.
   배포 실패 시에는 /rc-fix로 수정 후 새 태그를 따야 합니다.
   ```
6. **사용자의 "배포 성공" 확인을 반드시 대기.** 사용자가 명시적으로 "배포 성공" 또는 동등한 확인을 주기 전까지 **Phase 2로 절대 넘어가지 않는다.** 이 사이 대화가 이어지더라도 main PR 생성/머지 관련 동작은 수행하지 않는다.

#### Phase 2 — main 머지 (사용자의 배포 성공 확인 이후에만)

7. **main으로 PR 생성:**
   ```
   gh pr create --base main --head rc/{version} \
     --title "release: {release_version}" \
     --body "RC {version} → main 배포 (태그: {release_version}) — KT 실서버 배포 성공 확인 완료"
   ```
8. PR URL 출력 후 사용자에게 머지 요청. 사용자가 머지 완료했다고 알릴 때까지 대기
9. main 머지 확인 후 완료 메시지 + `/release-complete` 안내
...
실배포 2단계. main 머지 후 머지된 PR을 모아 GitHub Releases에 릴리즈 노트를 드래프트로 게시한다.

1. **GitHub Releases 드래프트 자동 생성:**
   - `/rc-notes {version} --post {release_version}` 절차(이 문서의 `/rc-notes` 섹션)를 그대로 실행
   - 즉, 머지된 PR을 수집해 마크다운 노트를 생성하고 `{release_version}` 태그에 **드래프트**로 게시
   - 기존 드래프트가 이미 있으면 본문 덮어쓰기 확인 절차를 따름
   - 드래프트 URL을 출력하고 "GitHub UI에서 Publish release로 공개하세요" 안내
   > ℹ️ 항상 `--draft`. publish는 사용자가 수동으로 수행.
2. 완료 메시지 + `/release-sync` 안내
...
실배포 3단계. 배포 후 운영 기준(main)을 develop에 되돌려 동기화하고, 두 브랜치가 일치하는지 확인한다.

1. **develop으로 PR 생성 (동기화):**
   ```
   ...
   ```
2. PR URL 출력 후 사용자에게 머지 요청. 사용자가 머지 완료했다고 알릴 때까지 대기
3. 최종 `git fetch origin && git diff origin/develop origin/main` 동기화 확인 (diff가 비어 있어야 정상)
4. 배포 완료 메시지 출력
