...
| 배포 | `/release 2.1.0.0` | RC 태깅 → 실배포 성공 확인 → main/develop 머지 |
...
RC에서 배포 태그를 먼저 생성해 이미지 빌드/실서버 배포까지 검증한 후, main과 develop으로 PR 머지하여 동기화한다.

**핵심 원칙**: 배포 태그 생성 → 이미지 빌드 → 실서버(KT) 배포 성공 확인 → main/develop 머지.
배포 검증 이전에는 **절대 main/develop을 변경하지 않는다.** 실배포가 실패하면 해당 태그는 폐기하고 `/rc-fix`로 수정 후 새 태그를 따서 재시도한다.
...
5. **이미지 빌드 및 KT 실서버 배포 안내 출력:**
   ...
   ⚠️ 배포 성공 확인 전까지 main/develop은 건드리지 않습니다.
   배포 실패 시에는 /rc-fix로 수정 후 새 태그를 따야 합니다.
   ```
6. **사용자의 "배포 성공" 확인을 반드시 대기.** 사용자가 명시적으로 "배포 성공" 또는 동등한 확인을 주기 전까지 **Phase 2로 절대 넘어가지 않는다.** 이 사이 대화가 이어지더라도 main/develop PR 생성/머지 관련 동작은 수행하지 않는다.

#### Phase 2 — main / develop 머지 (사용자의 배포 성공 확인 이후에만)
...
9. **main 머지 확인 후 GitHub Releases 드래프트 자동 생성:**
...
10. **develop으로 PR 생성 (동기화):**
    ...
    ```
11. PR URL 출력 후 사용자에게 머지 요청. 사용자가 머지 완료했다고 알릴 때까지 대기
12. 최종 `git fetch origin && git diff origin/develop origin/main` 동기화 확인
13. 배포 완료 메시지 출력
